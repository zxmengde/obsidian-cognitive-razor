...
var VectorIndex = class {
  ...
  async upsert(entry) {
    try {
      ...
      const conceptVector = {
        ...
        metadata: {
          createdAt: isUpdate ? this.indexMeta.concepts[entry.uid]?.lastModified || now : now,
          updatedAt: now,
  ...
  async search(type, embedding, topK) {
    try {
      ...
      const searchResults = topResults.map((r) => ({
        uid: r.vector.id,
  ...
  async searchAboveThreshold(type, embedding, threshold) {
    try {
      ...
      const searchResults = matched.map((r) => ({
        uid: r.vector.id,
        similarity: r.similarity,
        name: this.cruidCache?.getName(r.vector.id) || r.vector.id,
        path: this.cruidCache?.getPath(r.vector.id) || ""
...
function generateFilePath(standardName, directoryScheme, type) {
  ...
  return (0, import_obsidian3.normalizePath)(path);
}
...
function buildVerificationReportMarkdown(result) {
  ...
  if (typeof assessment === "string" && assessment) {
    const label = assessment === "pass" ? "\u2705 \u901A\u8FC7" : assessment === "needs_review" ? "\u26A0\uFE0F \u9700\u8981\u5BA1\u67E5" : assessment === "fail" ? "\u274C \u672A\u901A\u8FC7" : assessment;
...
var WorkbenchView = class extends import_obsidian7.ItemView {
  cleanup = null;
  plugin;
...
var DiffTabView = class extends import_obsidian8.ItemView {
  cleanup = null;
  plugin;
  viewState = null;
  /** 标记用户是否已做出决策（接受/拒绝） */
...
// src/ui/svelte/settings-tab.ts
var CRSettingTab = class extends import_obsidian9.PluginSettingTab {
  cleanup = null;
  plugin;
...
function shouldSuppress(level, message) {
  ...
  const now = Date.now();
  if (lastNotice.level === level && lastNotice.message === message && now - lastNotice.timestamp < DEDUPE_WINDOW_MS) {
    return true;
  }
  return false;
}
...
function showSuccess(message) {
  ...
  new import_obsidian10.Notice(message, DURATION.success);
}
function showWarning(message) {
  if (shouldSuppress("warning", message)) return;
  recordNotice("warning", message);
  new import_obsidian10.Notice(message, DURATION.warning);
}
function showError(error, fallback2) {
  ...
  new import_obsidian10.Notice(message, DURATION.error);
}
...
// src/ui/svelte/setup-wizard.ts
var SetupWizard = class extends import_obsidian11.Modal {
  plugin;
  cleanup = null;
  constructor(app, plugin) {
    super(app);
...
var CognitiveRazorPlugin = class extends import_obsidian12.Plugin {
  /** 服务容器 */
  container;
  ...
  showSafeMode(error) {
    ...
    new import_obsidian12.Notice(`Cognitive Razor \u521D\u59CB\u5316\u5931\u8D25\uFF0C\u5DF2\u8FDB\u5165\u5B89\u5168\u6A21\u5F0F: ${errorMessage}`, 1e4);
    this.addSettingTab(new CRSettingTab(this.app, this));
  }
  // ========================================================================
  // 辅助方法
  ...
  handleNetworkStatusChange(online, reason) {
    ...
    if (online) {
      if (this.isOffline) {
        ...
        if (t) new import_obsidian12.Notice(t.notices.networkRestored);
      }
      return;
    }
    if (!this.isOffline) {
      ...
      if (t) {
        new import_obsidian12.Notice(reason ? `${t.notices.networkOffline}: ${reason}` : t.notices.networkOffline);
      }
    }
  }
  /**
