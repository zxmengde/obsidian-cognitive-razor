...
> 
> **1.1.0 变更说明**：
> - 简化 Provider 系统：统一使用 OpenAI 标准格式，通过 baseUrl 兼容其他服务
> - 简化迁移系统：改为轻量级版本兼容性检查
> - 更新文件目录结构以匹配实际实现
> - 完善设置模型定义
...
**O-05 版本兼容性（Version Compatibility）**
> 插件升级时必须检查数据兼容性；不兼容时提示用户并提供迁移选项。

- **来源**：O-04 配置持久化
- **实现**：
  - 设置中存储 `version` 字段标识数据版本
  - 插件加载时检查版本兼容性
  - 不兼容时显示提示，提供导出/重置选项
  - 兼容性变更记录在 CHANGELOG.md 中
...
**A-FUNC-06 配置与版本管理（Configuration & Version Management）**
> 配置存储在插件目录，含版本号；升级时检查兼容性并提示用户。

- **来源**：O-04, O-05
- **配置版本**：存储在 `settings.version` 字段
- **兼容性检查**：加载时比较版本，不兼容时提示用户选择迁移或重置
...
- **存储结构**：
  ...
  ```
- **注意**：插件配置使用 Obsidian 标准的 `data.json`（由 `loadData/saveData` 管理）
...
interface PluginSettings {
  ...
  // 功能开关
  enableGrounding: boolean;           // 启用事实核查，默认 false
  
  // Provider 配置（使用 OpenAI 标准格式，通过 baseUrl 兼容其他服务）
  providers: Record<string, ProviderConfig>;
  
  // 默认 Provider ID
  defaultProviderId: string;
  
  // 任务模型配置（每种任务类型可使用不同的模型）
...
/**
 * Provider 配置
 * 统一使用 OpenAI 标准格式，通过 baseUrl 兼容 OpenRouter、Azure、本地模型等
 */
interface ProviderConfig {
  type: "openai";                     // Provider 类型（固定为 openai 标准格式）
  apiKey: string;                     // API 密钥
  baseUrl?: string;                   // 自定义端点（默认 https://api.openai.com/v1）
  defaultChatModel: string;           // 默认聊天模型（如 gpt-4o）
  defaultEmbedModel: string;          // 默认嵌入模型（如 text-embedding-3-small）
...
interface TaskModelConfig {
  providerId: string;                 // Provider ID（引用 providers 中的配置）
  model: string;                      // 模型名称
...
> 所有生成类任务若存在 CTX_VAULT，必须将 vault_index 限定为与目标类型一致的节点；相似度 ≥ similarityThreshold 时必须记录为重复对并展示在 DuplicatesPanel。
...
> 每个状态变更、锁变更、写入操作必须产生日志事件；任务失败需附带错误码。
...
- **日志事件类型**：
  ...
  - VERSION_CHECK / VERSION_MISMATCH
  - API_REQUEST / API_RESPONSE

**TA-07 版本兼容性检查（Version Compatibility Check）**
> 版本不兼容时提示用户并提供迁移选项，不强制阻断加载。

- **来源**：O-05 版本兼容性
- **实现**：
  - 插件 onload 时检查 `settings.version`
  - 版本不兼容时显示模态对话框，提供选项：
    - 尝试自动迁移（如果可行）
    - 导出当前数据后重置
    - 继续使用（可能有风险）
...
| VersionChecker | 检查版本兼容性，提示迁移选项 | settings.version | 兼容性检查结果 |
...
| CHANGELOG | 版本变更记录 | CHANGELOG.md | Markdown |
...
interface IUndoManager {
   ...
   * @returns 快照 ID
   */
  createSnapshot(filePath: string, content: string, operation: string): Promise<Result<string>>;
...
├── manifest.json              # 插件清单
├── styles.css                 # 样式文件
├── data.json                  # Obsidian 标准配置文件（由 loadData/saveData 管理）
│
├── data/                      # 插件数据目录
│   ├── queue/
│   │   └── tasks.json         # 队列状态
...
| 3. 配置 Provider | 输入 API 端点和密钥（支持 OpenAI、OpenRouter、Azure 等兼容服务） | 显示配置表单 | A-UCD-04 |
...
| UC-12 | 版本不兼容 | 升级到新版本 | 显示兼容性提示，提供迁移选项 | A-FUNC-06, TA-07 |
...
| M-NF-08 | 版本兼容性检查成功率 | 100% | 版本升级测试 | A-NF-02 |
...
| similarityThreshold | A-FUNC-04 | A-PDD-07 | Journey-Dedup | TA-03 | ✓ |
...
**T-TAD-03 版本兼容性定理**
> 前提：TA-07, A-FUNC-06  
> 结论：版本升级时用户有足够信息做出安全决策。

证明思路：
1. 由 TA-07，版本不兼容时显示提示对话框
2. 由 A-FUNC-06，提供导出/重置/继续等选项
3. 用户可以在了解风险后做出选择
4. 因此，升级过程是可控的 ∎
...
| Dedup | 去重，检测语义相似的概念 |
| Provider | AI 服务提供商（使用 OpenAI 标准格式，通过自定义端点兼容其他服务） |
...
| 1.1.0 | 2025-12-06 | 简化 Provider 系统和迁移系统，更新文件结构 |
