...
6. 选择 OpenRouter 支持的模型（如 `anthropic/claude-3-opus`）

> **提示**：从 v2.0.0 开始，OpenRouter 不再作为独立 Provider。使用 OpenAI Provider + 自定义端点即可访问 OpenRouter 的所有模型。
...
3. 在配置向导中选择 **OpenAI**
4. 输入任意 API Key（如 `ollama`）
5. 在"自定义端点"字段中输入：`http://localhost:11434/v1`
6. 模型名称输入：`llama2`

**使用 LM Studio**：

1. 安装并启动 LM Studio
2. 在 LM Studio 中加载模型并启动本地服务器
3. 在配置向导中选择 **OpenAI**
4. 输入任意 API Key（如 `lmstudio`）
5. 在"自定义端点"字段中输入：`http://localhost:1234/v1`
6. 模型名称输入 LM Studio 中显示的模型名称

**使用其他本地服务**：

只要服务实现了 OpenAI 兼容的 API 接口，都可以使用：

1. 确保服务已启动并监听端口
2. 选择 **OpenAI** Provider
3. 输入服务要求的 API Key（如果不需要可输入任意值）
4. 输入服务的端点 URL
5. 输入模型名称
...
3. **自定义端点错误**
   - 确认 URL 格式正确（必须以 `http://` 或 `https://` 开头）
   - 本地服务确保已启动并监听正确端口
   - 检查端点路径是否正确（通常以 `/v1` 结尾）
...
- 点击"测试连接"按钮查看详细错误信息
- 查看日志文件：`.obsidian/plugins/obsidian-cognitive-razor/data/logs/`
- 使用浏览器开发者工具（`Ctrl/Cmd + Shift + I`）查看网络请求
