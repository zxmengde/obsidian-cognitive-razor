...
<task_instruction>
聚焦于 `<concept_info>` 中该理论的内部组件分解，生成最小充分集——每一项都必须论证其不可或缺性：去掉它，理论的推理链就会断裂。

生成策略：
1. sub_theories 严格遵循 MECE 原则。每个子理论聚焦于"它负责解释什么？去掉它，母理论的哪一段推理链会断裂？"
2. entities 是理论成立所需的最小充分集——每个实体的 role 说明其不可替代的功能角色，attributes 聚焦于与理论直接相关的关键属性（不是百科全书式描述）。
3. mechanisms 解释实体间如何通过因果律产生联系——process 说明"什么作用于什么，产生什么变化"，function 说明"该机制在理论中解释了什么现象"。
